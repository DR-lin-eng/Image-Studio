...
func TestPreferredProbeModelsUsesAllModelsForResponses(t *testing.T) {
	text, image := preferredProbeModels("responses", []kernel.UpstreamModelDescriptor{
		{ID: "gpt-5.5"},
		{ID: "gpt-image-2"},
		{ID: "gpt-5.5"},
	})
	if len(text) != 2 || text[0].ID != "gpt-5.5" || text[1].ID != "gpt-image-2" {
		t.Fatalf("text=%v want all request models", text)
	}
