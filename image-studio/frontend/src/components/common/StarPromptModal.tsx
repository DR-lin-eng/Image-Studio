...
// 防止下次再弹。UI 按 OS 分:macOS / Apple 走液态玻璃 + 大圆角,Windows 走
// Fluent 紧凑 + 小圆角,Linux / 其他用通用中性样式(走 Modal 默认)。
export function StarPromptModal({ open }: { open: boolean }) {
  ...
  return (
    <Modal open={open} onClose={dismiss} width={isWindows ? 440 : 460}>
      <div className="flex flex-col items-center gap-4 py-1">
        {/* 头部图标:macOS 用环形液态玻璃光晕,Windows 用方形 Mica 卡片,
            Linux 通用 dashed accent。 */}
        <div
        ...
        <h3
          className={`text-center text-zinc-900 dark:text-zinc-50 ${
            isWindows ? "text-[17px] font-semibold tracking-[0]" : "text-[19px] font-semibold tracking-[-0.01em]"
          }`}
        ...
        <div
          className={`flex w-full items-center gap-2.5 border border-black/[0.06] bg-[var(--surface)] px-3 py-2 dark:border-white/[0.06] ${
            usesAppleUI ? "liquid-glass-panel rounded-[14px]" : isWindows ? "rounded-[10px]" : "rounded-[12px]"
          }`}
        >
        ...
        {/* 平台风味的脚注:macOS 给一行手写感,Windows 给一行紧凑提示,
            Linux 不显示(通用 modal 不主动加噪)。 */}
        {usesAppleUI && (
