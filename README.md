...
| **Android Phone APK** | `image-studio-android-phone-release.apk` | Android APK | WebView 壳层 + Android 手机 Material 3 前端 |
| **Android Pad APK** | `image-studio-android-pad-release.apk` | Android APK | WebView 壳层 + Android 平板 Material 3 自适应布局 |
...
Android 壳层工程位于 `android-shell/`。CI 会分别执行 `assemblePhoneRelease` 和 `assemblePadRelease`，两个 flavor 在构建时各自触发前端 `build:android` / `build:android-pad`，再把产物拷进 APK assets。
